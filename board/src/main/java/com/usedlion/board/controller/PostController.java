...
public class PostController {
    ...
    @GetMapping public List<Post> listAll() { return svc.getAll(); }
    @GetMapping("/{id}") public ResponseEntity<Post> getOne(@PathVariable Integer id) {
        return ResponseEntity.ok(svc.get(id));
    }
    @PostMapping public ResponseEntity<Post> create(@RequestBody Post p) {
        return ResponseEntity.ok(svc.create(p));
    }
    @PutMapping("/{id}") public ResponseEntity<Post> update(
            @PathVariable Integer id, @RequestBody Post p) {
